...
from convertish.converters import JSONStructureConverter
...
class TestJSONConverters(unittest.TestCase):
    ...
    def test_integer_to_json_conversion(self):
        ...
        for value, expected in value_expected:
            actual = JSONStructureConverter().from_type(type, value)
        ...
        for value, expected in value_expected:
            actual = JSONStructureConverter().to_type(type, value)
            self.assertEquals(actual,expected)
    ...
    def test_date_json_conversion(self):
        ...
        actual = JSONStructureConverter().to_type(type, value)
        self.assertEquals(actual,expected)
        self.assertRaises(ConvertError, JSONStructureConverter().to_type, type, 'nonsense')
        self.assertRaises(ConvertError, JSONStructureConverter().to_type, type, '1990-1-1andabit')
